...
class VOC_Data(object):
    def __init__(self, annotation_path, classes_path,  anchors_path=None, train=True, tree_classes_path=None, input_shape=(416, 416)):
        self.name = "voc_data"
        ...
        if tree_classes_path:
            self._tree_classes_path = tree_classes_path
            tree_classes = self._get_tree_class()
            dog_classes = np.array(tree_classes[[n.islower() for n in tree_classes]])
            cat_classes = np.array(tree_classes[[not n.islower() for n in tree_classes]])
            self.classes = np.concatenate([self.classes, cat_classes, dog_classes])
            self.cat_index = self.num_classes
            self.dog_index = self.num_classes + len(cat_classes)
            self.num_classes = len(self.classes)
        self.read_lines()
    ...
    def _get_tree_class(self):
        classes_path = os.path.expanduser(self._tree_classes_path)
        with open(classes_path) as f:
            class_names = f.readlines()
        class_names = [c.strip() for c in class_names]
        return np.array(class_names)
    ...
    def get_data_tree_label(self, annotation_line):
        element = annotation_line.split(" ")
        class_name = element[0].split("\\")[-1]
        tree_index = np.argmax([class_name.startswith(n) for n in self.classes])
    ...
    def get_data_label(self, annotation_line):
        ...
        h, w = self.input_shape
        if self.train:
            image = np.rot90(image, np.random.choice([0,1,2,3], 1, replace=False)[0])
        image = cv2.resize(image, (h, w), interpolation=cv2.INTER_CUBIC)
    ...
    def data_tree_label_generator(self, lines):
        all_images = []
        all_boxes = []
        for line in lines:
            image_data, box_data = self.get_data_tree_label(line)
            all_images.append(image_data)
            all_boxes.append(box_data)
        all_images = np.array(all_images)
        all_boxes = np.array(all_boxes)
    ...
    def read_data_label(self, batch_size):
        if self.train:
            batch_lines = np.random.choice(self.all_lines, size=batch_size, replace=False)
        else:
            batch_lines = self.all_lines
        image, label = self.data_label_generator(batch_lines)
        return image, label

    def read_data_tree_label(self, batch_size):
        batch_lines = np.random.choice(self.all_lines, size=batch_size)
        image, label = self.data_tree_label_generator(batch_lines)
        return image, label
...
if __name__ == '__main__':
    ...
    images, labels = dataset.read_data_label(2)
